...
int main(int argc, char* argv[])
{
  try
  {
    ...
    double T = 1e-00;                  // final computation time
    ...
    for(unsigned int i = 0; i < nBeads; i++)
    {
      ...
      (q0[i])->setValue(0, position_init + i * 2*R + R);
    ...
    beads[nBeads-1]->setComputeFExtFunction("plugins", "ballFExt");
    ...
    (*H)(0, 0) = 1.0;
    (*H)(1, 1) = 1.0;
    (*H)(2, 2) = 1.0;//fake tangent direction
    ...
    SP::SimpleMatrix HOfBeads(new SimpleMatrix(3, 2 * nDof));
    (*HOfBeads)(0, 0) = -1.0;
    (*HOfBeads)(0, 3) = 1.0;
    (*HOfBeads)(1, 1) = -1.0;
    (*HOfBeads)(1, 4) = 1.0;
    ...
    unsigned int outputSize = 1 + nBeads * 4+20;
    ...
    while(s->hasNextEvent())
    {
      ...
      for(unsigned int i = 0; i < nBeads - 1; i++)
      {
        // std::cout<< "abs(((beads[i])->q())->getValue(0) - R)" << abs(((beads[i])->q())->getValue(0) - R)<< std::endl;
        ...
        if(abs(((beads[i + 1])->q())->getValue(0) - ((beads[i])->q())->getValue(0) - 2 * R) < alert)
        {
          ...
          if(!interOfBeads[i].get())
          {
            ...
            nslaws[i].reset(new BinaryCohesiveNSL(e, 0, 0, sigma_c, delta_c,3));
      ...
      for(unsigned int i = 0; i < nBeads; i++)
      {
        dataPlot(k, idx++) = (beads[i]->q())->getValue(0);
        dataPlot(k, idx++) = (beads[i]->velocity())->getValue(0);
      }
