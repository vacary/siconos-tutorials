...
int update(double* q, vector<double> y, double h) {
  try {
    q[0] =
        (h - 1) * y[1] + (h + 3) * y[2] + 2 * h * y[0];  // For this example only ! NOT SAFE !
...
SimpleMatrix* compute_reference(double h, SolverOptions* options, NumericsMatrix* M) {
  try {
    ...
    (*dataPlot)(0, 4) = lambda20;                          // lambda2
                                   // data_plot[k, 5] = z0 computed afterward as it is
                                   // depending on \dot{lambda}
...
int compute_WeiestrassDiscretization(double h) {
  ...
  try {
    ...
    (*A)(0, 0) = -2.0;
    auto dyn(new FirstOrderLinearDS(init));
    dyn->setConstantA(A);
...
double compute_error2ref(double h, SimpleMatrix* reference, SolverOptions* options,
                         NumericsMatrix* M) {
  try {
    double T = 5.0;
    double t0 = 0.0;
    ...
    while (k < N) {
      ...
      while (comp_next_pwl) {
        ...
        if ((*reference)(j, 0) > tc) comp_next_pwl = false;
        if (j >= reference->size(0)) comp_next_pwl = false;
      }
    }
...
double compute_error_weiestrass2ref(double h, SimpleMatrix* reference) {
  try {
    ...
    std::shared_ptr<siconos::algebra::SiconosMatrix> A(new SimpleMatrix(dimX, dimX));
    (*A)(0, 0) = -2.0;
    auto dyn(new FirstOrderLinearDS(init, A));
    auto M(new SimpleMatrix(dimX, dimX));
    (*M)(0, 0) = 1.0;
    (*M)(1, 1) = 1.0;
    dyn->setMPtr(M);
...
int main(int argc, char* argv[]) {
  ...
  std::vector<double> htable(
      {1.00000000e-05, 1.08033152e-05, 1.16711619e-05, 1.26087241e-05, 1.36216020e-05,
       1.47158460e-05, 1.58979923e-05, 1.71751022e-05, 1.85548042e-05, 2.00453398e-05,
       2.16556124e-05, 2.33952406e-05, 2.52746159e-05, 2.73049642e-05, 2.94984134e-05,
       3.18680658e-05, 3.44280759e-05, 3.71937355e-05, 4.01815648e-05, 4.34094109e-05,
       4.68965549e-05, 5.06638264e-05, 5.47337285e-05, 5.91305720e-05, 6.38806207e-05,
       6.90122480e-05, 7.45561067e-05, 8.05453121e-05, 8.70156393e-05, 9.40057378e-05,
       1.01557362e-04, 1.09715619e-04, 1.18529241e-04, 1.28050875e-04, 1.38337396e-04,
       1.49450249e-04, 1.61455815e-04, 1.74425806e-04, 1.88437696e-04, 2.03575182e-04,
       2.19928686e-04, 2.37595891e-04, 2.56682330e-04, 2.77302012e-04, 2.99578103e-04,
       3.23643668e-04, 3.49642455e-04, 3.77729765e-04, 4.08073370e-04, 4.40854524e-04,
       4.76269038e-04, 5.14528453e-04, 5.55861305e-04, 6.00514488e-04, 6.48754729e-04,
       7.00870182e-04, 7.57172149e-04, 8.17996938e-04, 8.83707874e-04, 9.54697470e-04,
       1.03138977e-03, 1.11424288e-03, 1.20375170e-03, 1.30045090e-03, 1.40491810e-03,
       1.51777730e-03, 1.63970266e-03, 1.77142246e-03, 1.91372352e-03, 2.06745584e-03,
       2.23353771e-03, 2.41296118e-03, 2.60679802e-03, 2.81620607e-03, 3.04243618e-03,
       3.28683970e-03, 3.55087652e-03, 3.83612383e-03, 4.14428548e-03, 4.47720223e-03,
       4.83686269e-03, 5.22541521e-03, 5.64518076e-03, 6.09866670e-03, 6.58858186e-03,
       7.11785265e-03, 7.68964057e-03, 8.30736107e-03, 8.97470401e-03, 9.69565562e-03,
       1.04745224e-02, 1.13159567e-02, 1.22249846e-02, 1.32070362e-02, 1.42679775e-02,
       1.54141458e-02, 1.66523876e-02, 1.79900992e-02, 1.94352711e-02, 2.09965360e-02,
       2.26832196e-02, 2.45053971e-02, 2.64739529e-02, 2.86006458e-02, 3.08981791e-02,
       3.33802767e-02, 3.60617651e-02, 3.89586614e-02, 4.20882699e-02, 4.54692846e-02,
       4.91219013e-02, 5.30679382e-02, 5.73309663e-02, 6.19364499e-02, 6.69118990e-02,
       7.22870335e-02, 7.80939607e-02, 8.43673672e-02, 9.11447260e-02, 9.84665203e-02,
       1.06376485e-01, 1.14921870e-01, 1.24153719e-01, 1.34127175e-01, 1.44901815e-01,
       1.56541998e-01, 1.69117254e-01, 1.82702700e-01, 1.97379486e-01, 2.13235280e-01,
       2.30364794e-01, 2.48870348e-01, 2.68862481e-01, 2.90460612e-01, 3.13793754e-01,
       3.39001283e-01, 3.66233771e-01, 3.95653887e-01, 4.27437364e-01, 4.61774057e-01,
       4.98869069e-01, 5.38943979e-01, 5.82238167e-01, 6.29010244e-01, 6.79539592e-01,
       7.34128040e-01, 7.93101660e-01, 8.56812721e-01, 9.25641789e-01, 1.00000000e+00});
