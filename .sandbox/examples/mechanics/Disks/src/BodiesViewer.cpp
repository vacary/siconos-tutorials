...
void BodiesViewer::drawUCircle() {
  ...
  for (i = 0; i < Ne; i++) glVertex2fv(&circcoords[i][0]);
  glEnd();
...
void BodiesViewer::drawPar(float x1, float y1, float z1, float x2, float y2, float z2,
                           float w) {
  float dx = x2 - x1;
  float dy = y2 - y1;
  float dz = z2 - z1;
...
/* always visible */
void BodiesViewer::drawArrow(float x1, float y1, float z1, float x2, float y2, float z2,
                             float w) {
  float th, dx, dy, dz;
...
void BodiesViewer::drawSphere(float x, float y, float z, float theta, float phi, float psi,
                              float r, float *c) {
  float cl[3] = {0, 0, 0};
...
void BodiesViewer::drawSphere(float x, float y, float z, float a, float b, float c, float d,
                              float r, float *color) {
  GLUquadricObj *Sphere = gluNewQuadric();
  GLUquadricObj *Disk1 = gluNewQuadric();
    ...
    case SPHERE:
      ...
      switch (fig.type()) {
        case Type::LagrangianDS:
          drawSphere(GETX(ds), GETY(ds), GETZ(ds), GETA1(ds), GETA2(ds), GETA3(ds),
                     GETRADIUS(ds), c);
          break;
        case Type::NewtonEulerDS:
          drawSphere(GETX(ds), GETY(ds), GETZ(ds), GETA1(ds), GETA2(ds), GETA3(ds), GETA4(ds),
                     GETRADIUS(ds), c);
          break;
...
void BodiesViewer::drawSelectedQGLShape(const QGLShape &fig) {
  ...
  switch (fig.shape()) {
    case CIRCLE:
      ...
      drawArrow(GETX(ds), GETY(ds), GETX(ds) + Cal * GETXFE(ds) / dFe,
    ...
    case SPHERE:
      ...
      drawArrow(GETX(ds), GETY(ds), GETZ(ds), GETX(ds) + Cal * GETXFE(ds) / dFe,
                GETY(ds) + Cal * GETYFE(ds) / dFe, GETZ(ds) + Cal * GETZFE(ds), Cal / 10.);
      break;
    ...
    if (selectedName() >= 0) {
      if (!shapes_[selectedName()]->selected()) {
        ...
        switch (Type::value(*shapes_[selectedName()]->DS())) {
          case Type::NewtonEulerDS: {
            std11::static_pointer_cast<NewtonEulerDS>(shapes_[selectedName()]->DS())
                ->setConstantFext(fext);
            break;
          };
          case Type::LagrangianDS: {
            std11::static_pointer_cast<LagrangianDS>(shapes_[selectedName()]->DS())
                ->setConstantFext(fext);
            break;
          };
          default: {
      ...
      double massValue = ask<ForMassValue>(*shapes_[selectedName()]->DS());
...
QString BodiesViewer::helpString() const {
  ...
  text +=
      "Left and middle buttons pressed together rotate around the camera view direction "
      "axis<br><br>";
  text +=
      "Pressing <b>Alt</b> and one of the function keys (<b>F1</b>..<b>F12</b>) defines a "
      "camera keyFrame. ";
  text += "Simply press the function key again to restore it. Several keyFrames define a ";
  text +=
      "camera path. Paths are saved when you quit the application and restored at next "
      "start.<br><br>";
  text += "Press <b>F</b> to display the frame rate, <b>A</b> for the world axis, ";
  text += "<b>Alt+Return</b> for full screen mode and <b>Control+S</b> to save a snapshot. ";
  text += "See the <b>Keyboard</b> tab in this window for a complete shortcut list.<br><br>";
  text +=
      "Double clicks automates single click actions: A left button double click aligns the "
      "closer axis with the camera (if close enough). ";
  text +=
      "A middle button double click fits the zoom of the camera and the right button "
      "re-centers the scene.<br><br>";
  text +=
...
#ifdef QT_INTERFACE
BodiesViewer::BodiesViewer(QWidget *parent) : QGLViewer(parent) {};
#endif
