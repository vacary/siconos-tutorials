...
#include "UserDefinedParameter.hpp"
    ...
    //
    auto H(new SimpleMatrix(1, nDof));
    (*H)(0, 0) = 1.0;
    ...
    // --- Model ---
    // -------------
    auto impactingPunch(new NonSmoothDynamicalSystem(t0, T));
    ...
    auto OSI(new MoreauJeanOSI(theta, 0.5));
    ...
    auto s(new TimeStepping(impactingPunch, t, OSI, osnspb));
    ...
    std::shared_ptr<siconos::algebra::SiconosVector> q = punch->q();
    std::shared_ptr<siconos::algebra::SiconosVector> v = punch->velocity();
    std::shared_ptr<siconos::algebra::SiconosVector> p = punch->p(1);
    ...
    cerr << "Exception caught in ImpactingPunchTS.cpp" << endl;
    return 1;
  }
}
