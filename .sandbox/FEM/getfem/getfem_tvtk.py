...
def _getfem_to_tvtk_points(points):
    (N, nbpt) = points.shape
    if N < 3:
        points = numpy.concatenate((points, numpy.zeros([3 - N, nbpt])), axis=0)
...
class FigureItem:
    ...
    def deformation_from_mf(self, mf, U, scale):
        ...
        P = P + scale * deform
        self.sl.set_pts(P)
        # print "deformation!", repr(mf), U.size(),
        #  repr(self.sl), "\nDEFORM=",self.deform,"\n"
        sys.stdout.flush()
    ...
    def vtk_actors(self):
        if self.actors is None:
            ...
            if self.show_edges:
                ...
                if Pe.size:
                    E = numpy.array(
                        numpy.concatenate((E1.transpose(), E2.transpose()), axis=0), "I"
                    )
                    edges = tvtk.PolyData(points=_getfem_to_tvtk_points(Pe), polys=E)
...
class Figure:
    ...
    def show_mesh_fem(self, mf, **args):
        ...
        if "data" in args:
            it.set_scalar_data(args.get("data"), args.get("scalar_label", "data"))
