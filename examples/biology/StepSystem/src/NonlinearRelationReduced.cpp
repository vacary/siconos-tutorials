...
user_defined::NonlinearRelation::NonlinearRelationReduced() : FirstOrderType2R{} {
  setComputehFunction(
      [](const siconos::algebra::BlockVector& state,
         const Eigen::Ref<const siconos::algebra::SiconosVector>& lambda,
         Eigen::Ref<siconos::algebra::SiconosVector> y) { y(0) = 8.0 - x(1); });

  setComputegFunction([](const Eigen::Ref<const siconos::algebra::SiconosVector>& lambda,
                         siconos::algebra::BlockVector& res) {
    res.setValue(1, 40.0 * (1 - lambda(0)));
    res.setValue(0, 0.0);
  });
}

void user_defined::NonlinearRelation::NonlinearRelationReduced::initialize(
    Interaction& inter) {
  ...
  (*jacobianhOver_state_view_)(0, 0) = 0.0;
  (*jacobianhOver_state_view_)(1, 0) = -40.0;
  ...
  (*jacobiangOver_lambda_view_)(0, 0) = 0.0;
  (*jacobiangOver_lambda_view_)(1, 0) = -40.0;
