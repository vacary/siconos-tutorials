...
# options.iparam[sn.solver_ids.SICONOS_FRICTION_3D_ADMM_IPARAM_SYMMETRY] =
#   sn.solver_ids.SICONOS_FRICTION_3D_ADMM_FORCED_ASYMMETRY
...
rock_velocity = [-10, 0, -35.0, 0.5, 10.0, 0.1]
...
def one_rock(
    io, name, cname, rock_size=0.05, density=1, trans=None, velo=None, tob=None
):
    ...
    def vert(id1, id2, a, b, c):
        return (
            a * math.cos(rd[id1]) * math.cos(rd[id2]),
...
with MechanicsHdf5Runner(io_filename=fn) as io:
    ...
    io.add_primitive_shape(
        "Half_Box", "Box", [width, depth / 2.0, height], outsideMargin=margin
    )
    ...
    def make_large_stack_with_straight_side(
        X, Y, N, M, W, orientation=[1.0, 0.0, 0.0], density=1.0
    ):
        """
        N is the depth of the wall
        M is the width
    ...
    if configuration == "tall_wall":
        ...
        make_large_stack(0, 0, 1, 20, 5, density=2300)
        make_large_stack_with_straight_side(
            10, -1.5 * depth, 1, 4, 5, orientation=[0.0, 1.0, 0.0], density=2300
        )
        make_large_stack_with_straight_side(
            5, -1.5 * depth, 1, 4, 5, orientation=[0.0, 1.0, 0.0], density=2300
        )
        make_large_stack_with_straight_side(
            0, -1.5 * depth, 1, 4, 5, orientation=[0.0, 1.0, 0.0], density=2300
        )
        make_large_stack_with_straight_side(
            -5, -1.5 * depth, 1, 4, 5, orientation=[0.0, 1.0, 0.0], density=2300
        )
