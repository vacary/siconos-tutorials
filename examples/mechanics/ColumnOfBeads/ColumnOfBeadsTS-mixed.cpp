...
int main(int argc, char* argv[])
{
  try
  {
    ...
    SP::NonSmoothLaw nslaw_2(new NewtonImpactFrictionNSL(e, 0.0, 0.0,  2));
      ...
      for(unsigned int i = 0; i < nBeads - 1; i++)
      {
        ...
        if(abs(((beads[i + 1])->q())->getValue(0) - ((beads[i])->q())->getValue(0) - 2 * R) < alert)
        {
          ...
          if(!interOfBeads[i].get())
          {
            ...
            interOfBeads[i].reset(new Interaction(nslaw_2, relationOfBeads[i]));
    ...
    ioMatrix::write("ColumnOfbeadsTS-mixed.dat", "ascii", dataPlot, "noDim");
