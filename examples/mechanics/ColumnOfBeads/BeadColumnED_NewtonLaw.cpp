  ...
  try {
    ...
    for (auto id = 0; id < NumberBalls; ++id) {
      ...
      VecOfallDS[id] = std::make_shared<siconos::modeling::LagrangianLinearTIDS>(
          q0Ball[id], vel0Ball[id], MassBall[id], siconos::algebra::copy_t);
    ...
    std::vector<std::shared_ptr<siconos::modeling::NewtonImpactNSL>> nslaws(NumberContacts);
    ...
    Hwall_ball(0, 0) = 1.;
    E[0] << -RadiusBalls(0);
    nslaws[0] = std::make_shared<siconos::modeling::NewtonImpactNSL>(ResCofContacts(0));
    ...
    for (auto id = 1; id < NumberContacts; ++id) {
      ...
      nslaws[id] = std::make_shared<siconos::modeling::NewtonImpactNSL>(ResCofContacts(id));
