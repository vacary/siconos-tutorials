...
int main(int argc, char* argv[]) {
  try {
    ...
    auto pStart = std::make_shared<MVertex>(0, 0.0, 0.0, 0.0);
    auto pEnd = std::make_shared<MVertex>(0, 0.0, 4.0, 0.0);
    // auto pEnd = std::make_shared<MVertex>(0, 4.0, 0.0, 0.0);
    // int nbBeams = 1;
    int nbBeams = 4;
    ...
    auto nslawContact = std::make_shared<siconos::modeling::NewtonImpactFrictionNSL>(0, 0, 0, nslawdimension);
    ...
    if (nslawdimension == 3)
      (*H_contact)(2, Hindex+2) = -1.0/conditionning;
    ...
    auto relation_contact = std::make_shared<siconos::modeling::LagrangianLinearTIR>(H_contact, b_contact);
    std::cout << "LagrangianLinearTIR created " << std::endl;

    auto inter_contact = std::make_shared<siconos::modeling::Interaction>(nslawContact, relation_contact);
    ...
    osnspb->numericsSolverOptions()->iparam[SICONOS_FRICTION_3D_IPARAM_ERROR_EVALUATION] =  SICONOS_FRICTION_3D_NSGS_ERROR_EVALUATION_FULL;
    ...
    auto p1 = block->p(1);
    auto epsilonPointP = beam->plasticRate();
    auto epsilonPointPOld = std::make_shared<siconos::algebra::SiconosVector>(beam->plasticRate()->size());
    epsilonPointPOld->fill(0);
    auto diff = std::make_shared<siconos::algebra::SiconosVector>(beam->plasticRate()->size());
    auto epsilonE = std::make_shared<siconos::algebra::SiconosVector>(beam->plasticRate()->size());
    auto epsilon = std::make_shared<siconos::algebra::SiconosVector>(beam->plasticRate()->size());
    ...
    auto vBallMass = std::make_shared<siconos::algebra::SiconosVector>(block->velocity()->size());
    auto vTotal = std::make_shared<siconos::algebra::SiconosVector>(beam->velocityDimension()+block->velocity()->size());
    ...
    v->toBlock(*vTotal, beam->velocityDimension(), 0, 0);
    vball->toBlock(*vTotal, block->velocity()->size(), 0, beam->velocityDimension());  //vTotal = [vTree; vBlock]
    ...
    double energieCinetiqueBlock, dissipationContact= 0, dissipationPlastique = 0, energieMecanique, energieCinetiqueTree;
    auto filename =
    ...
    while (s->hasNextEvent()) {
      ...
      dataPlot(k, 9) = energieCinetiqueBlock+energieMecanique+energieCinetiqueTree+dissipationPlastique+dissipationContact;
      dataPlot(k, 10) = (*p1)(0);
