...
int main(int argc, char* argv[])
{
  try
  {
    ...
    //std::shared_ptr<Mesh> mesh (createMeshFromGMSH2("./mesh_data/tetra_simple.msh"));    // simple reference tetrahedron for debugging TH4 element
    //std::shared_ptr<Mesh> mesh (createMeshFromGMSH2("./mesh_data/tetra_simple2.msh"));   // simple tetrahedron for debugging TH4 element from Felippa's book
    //std::shared_ptr<Mesh> mesh (createMeshFromGMSH2("./mesh_data/cube.msh"));            // simple cube example
    std::shared_ptr<Mesh> mesh(createMeshFromGMSH2("./mesh_data/cube_multi.msh"));         // simple beam example with a symmetric mesh
    //std::shared_ptr<Mesh> mesh (createMeshFromGMSH2("./mesh_data/beam.msh"));            // simple beam example
    ...
    //std::shared_ptr<Material> mat1 = std::make_shared<Material>(0.0, 96, 1./3.);
    ...
    std::shared_ptr<FiniteElementLinearTIDS> FEsolid(new FiniteElementLinearTIDS(mesh, materials, Siconos::SPARSE));
    ...
    std::shared_ptr<NonSmoothDynamicalSystem> solid(new NonSmoothDynamicalSystem(t0, T));
    ...
    std::shared_ptr<SiconosVector>  initial_gap = std::make_shared<SiconosVector>(1, Lz*1e-05);
