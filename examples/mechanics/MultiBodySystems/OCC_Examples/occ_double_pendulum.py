...
cylinder1 = BRepPrimAPI_MakeCylinder(0.3, l1).Shape()
cylinder2 = BRepPrimAPI_MakeCylinder(0.3, l2).Shape()
ground = BRepPrimAPI_MakeBox(gp_Pnt(-50, -50, 0), 100.0, 100.0, 0.5).Shape()
brick = BRepPrimAPI_MakeBox(gp_Pnt(-bx / 2.0, -by / 2.0, -bz / 2.0), bx, by, bz).Shape()
...
with MechanicsHdf5Runner(config=config_occ) as io:
    ...
    io.add_object(
        "arm1",
        [
            Contactor("Mass1", contact_type="Face", contact_index=0),
            Contactor(
                "Arm1",
                contact_type="Face",
                contact_index=0,
    ...
    io.add_object(
        ...
        [
            Contactor(
                "Brick",
                instance_name="BallContact",
                contact_type="Face",
                contact_index=0,
            )
        ],
        translation=[0, -2 * r2, r2 + gap + 0.5],
        inertia=np.eye(3),
        mass=m2,
    )
