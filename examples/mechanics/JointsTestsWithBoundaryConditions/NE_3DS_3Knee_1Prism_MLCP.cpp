...
int main(int argc, char* argv[])
{
  try
  {
    ...
    SP::NewtonEulerDS beam2= std::make_shared<siconos::modeling::NewtonEulerDS>(q02, v02, m, I2));
    // -- Set external forces (weight) --
    ...
    SP::NewtonEulerDS beam3= std::make_shared<siconos::modeling::NewtonEulerDS>(q03, v03, m, I3));
    // -- Set external forces (weight) --
    ...
    //auto bd = std::make_shared<siconos::modeling::BoundaryCondition>(bdindex,bdPrescribedVelocity));
    ...
    P->zero();
    P->setValue(0, L1 / 2);
    SP::KneeJointR relation2= std::make_shared<siconos::joints::KneeJointR>(P, false, beam1, beam2));
    ...
    P->zero();
    P->setValue(0, -L1 / 2);
    SP::KneeJointR relation3= std::make_shared<siconos::joints::KneeJointR>(P, false, beam2, beam3));

    auto nslaw1= std::make_shared<siconos::modeling::EqualityConditionNSL>(relation1->numberOfConstraints()));
    auto nslaw2= std::make_shared<siconos::modeling::EqualityConditionNSL>(relation2->numberOfConstraints()));
    auto nslaw3= std::make_shared<siconos::modeling::EqualityConditionNSL>(relation3->numberOfConstraints()));
    ...
    axe1->zero();
    axe1->setValue(0, 1);
    SP::PrismaticJointR relation4= std::make_shared<siconos::joints::PrismaticJointR>(axe1, false, beam3));
    ...
    autoOSI2= std::make_shared<siconos::integrators::MoreauJeanOSI>(theta));
    ...
    if((error=siconos::algebra::io::compareRefFile(dataPlot, "NE_3DS_3Knee_1Prism_MLCP.ref",
                                       eps)) >= 0.0
        && error > eps)
      return 1;
