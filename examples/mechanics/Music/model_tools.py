...
def build_frets_from_file(
    string, restitution_coeff, matlab_frets_file, filt, visu=False
):
    ...
    for ifret in range(nb_frets):
        frets.append(
            Fret(
                string,
                contact_positions=(i_frets[ifret], y_frets[ifret]),
...
def save_simu_to_hdf5(model, ds, filename, matlab_data, filt_frets, restit):
    ...
    time_steps = h5file.create_dataset("times", time_shape, dtype=np.float64)
    dof_shape = model.data_ds[ds].shape
    ds_states = h5file.create_dataset("dof", dof_shape, dtype=np.float64)
    ...
    if model.save_interactions:
        ...
        ishape = (
            model.nb_time_steps_output + 1,
            model.interactions_output,
        )  # assume inter size == 1
        for ic in range(nb_inter):
            interaction = interactions[ic]
            nbc = interaction.dimension()
            inter_states[ic] = h5file.create_dataset(
                "contact_" + str(ic), ishape, dtype=np.float64
            )
...
def load_model(filename, visu=True):
    ...
    * load simulation results
    ...
    string = StringDS(
        n_modes,
        geometry_and_material=G_string,
...
def compute_ak1(omega2, sigma, dt):
    ak = 2.0 * np.exp(-dt * sigma) * np.cos(dt * np.sqrt(omega2 - sigma**2))
    return ak
...
def compute_dtsigmastar(omega2, sigma, dt):
    ...
    thetak = 2.0 / omega2dt2 - ak / (1 + ek - ak)
    dtsigmastar = (1.0 + (1.0 - thetak) * omega2dt2 * 0.5) * (1 - ek) / (1 + ek)
...
def compute_fd_dtsigmastar(omega2, sigma, dt):
    ...
    dtsigmastar = dtsig
    dtsigmastar += dtsig * dt2omega2 / 12.0
    buff1 = dt2omega2 / 240 - dtsig**2 / 180
    buff1 *= dt2omega2 * dtsig
    dtsigmastar += buff1
    c7 = (
        dt2omega2**3 * dtsig / 6048
        - dt2omega2**2 * dtsig**3 / 1512.0
        + dt2omega2 * dtsig**5 / 1890.0
    )
    print("neg ", c7)
    return dtsigmastar
...
def compute_fd_invwkk(omega2, sigma, dt):
    ...
    dt2omega2 = dt**2 * omega2
    fd_inv = 1 - dtsig
    fd_inv += 2.0 / 3 * dtsig**2 - dt2omega2 / 12.0
    fd_inv += 1.12 * dt2omega2 * dtsig
    fd_inv -= dtsig**3 / 3.0
    fd_inv += (dt2omega2 / (6.0 * np.sqrt(10))) ** 2
    fd_inv -= (dt2omega2 / 20.0) * dtsig**2
    fd_inv += (dtsig**4) * 2.0 / 15.0
    fd_inv -= (dt2omega2 / (6.0 * np.sqrt(10))) ** 2 * dtsig
    fd_inv += (dtsig**3) / 45 * dt2omega2
    fd_inv -= (dtsig**5) / 45
    fd_inv += (
        dt2omega2**3 / 20160.0
        + dt2omega2**2 * dtsig**2 / 630.0
        - dt2omega2 * dtsig**4 / 126.0
        + 4.0 * dtsig**6 / 315.0
    )
    c7 = (
        dt2omega2**3 * dtsig / 20160.0
        - dt2omega2**2 * dtsig**3 / 1512.0
        + dt2omega2 * dtsig**5 / 420.0
        - dtsig**7 / 315.0
    )
    print("neg ", c7)
    return fd_inv
