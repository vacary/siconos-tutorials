...
class StringDS(sm.LagrangianLinearDiagonalDS):
    ...
    def _compute_initial_state_std(self, max_coords):
        ...
        u0 += [slope * (i * dx - self.length) for i in range(imax, self.n_modes + 2)]
    ...
    def eigenfreq(self, j):
        ...
        return 0.5 * j * self.c0 / self.length * np.sqrt(1 + self.stiffness_coeff * j)
    ...
    def read_linear_coeff(self, radix):
        ...
        if os.path.exists(damp_file):
            ...
            damping_mat = 2.0 * npw.asrealarray(sigmas)
        else:
    ...
    def compute_linear_coeff(self):
        ...
        omega2 = npw.asrealarray(
            [1.0 + self.stiffness_coeff * (j**2) for j in range(1, self.n_modes + 1)]
        )
        ...
        # 2.S.Gamma.S-1
        sigma = self.compute_damping(np.sqrt(omega2) / (2.0 * math.pi))
    ...
    def compute_damping(self, nu):
        ...
        r *= 2.0 * math.pi
        indices = np.arange(1, self.n_modes + 1)
        q_air_inv = r / (2.0 * math.pi * self.density * nu)
...
class Fret(sm.Interaction):
    ...
    * Lagrangian Linear Time-invariant relation
    ...
    def __init__(self, string, contact_positions, restitution_coeff):
        ...
        relation = sm.LagrangianLinearTIR(hmat, [dist])
...
class Guitar(object):
    ...
    def __init__(
        ...
        default_integrator: siconos.integrators.OneStepIntegrator
        ...
        * For integrators:
        ...
        self.nsds = sm.NonSmoothDynamicalSystem(time_range[0], time_range[1])
        ...
        # -- Simulation --
        moreau_bilbao = siconos.integrators.MoreauJeanBilbaoOSI()
        moreau_jean = siconos.integrators.MoreauJeanOSI(0.500001)
        ...
        t = siconos.simulation.TimeDiscretisation(t0, self.time_step)
        self.nb_time_steps = (int)((tend - t0) * fs)
        self.output_freq = output_freq
        self.nb_time_steps_output = (int)(self.nb_time_steps / output_freq)
        # (3) one step non smooth problem
        self.osnspb = siconos.nonsmooth_formulations.LCP()
    ...
    def plot_modes(self, ds, times=None, plot_shape=None, filename=None, iplot=1):
        """Plot collected data (positions ...) of a dynamical system
        ...
        for k in range(nb_points):
            ...
            ylimits = (ymin - 0.2 * abs(ymin), 1.1 * data[:, time_ind[k]].max())
            for ic in range(nbc):
                vpos = -interactions[ic].relation().e()[0]
                plt.plot(
                    (interactions[ic].contact_pos, interactions[ic].contact_pos),
                    (2.0 * vpos, vpos),
                    "o-",
                )
            plt.ylim(ylimits)
    ...
    def interactions_linked_to_ds(self, ds):
        ...
        ds.
        """
        return [k for k in self.strings_and_frets if self.strings_and_frets[k] in [ds]]
    ...
    def make_movie(self, ds, movie_name, sampling=100):
        ...
        self.convert_modal_output(ds)
        data = self.data_ds[ds]
        ylimits = (data.min() - 0.2 * abs(data.min()), 1.1 * data.max())
    ...
    def contactogram(self, ds, nfig=12):
        ...
        for ic in range(nb_inter):
            ...
            contact_indices = np.where(
                self.data_interactions[inter][1][:] > 0.0
            )  # 1e-8)
            nbcontacts = len(contact_indices[0])
            pos = inter.contact_pos
            plt.subplot(1, 2, 1)
            plt.plot(
                self.time[contact_indices[0]],
                [
                    pos,
                ]
                * nbcontacts,
