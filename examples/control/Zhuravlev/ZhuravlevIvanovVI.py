...
def vi_nabla_function(n, l, nabla_Fmcp):
    ...
    nabla_Fmcp[0, 0] = -h * (
        _kappa * l1 * v_gamma + (-1.0 + gamma * h * _kappa * l0 * l1) * nabla_Fmcp[1, 0]
    )
    nabla_Fmcp[0, 1] = -h * (
        _kappa * l0 * v_gamma + (-1.0 + gamma * h * _kappa * l0 * l1) * nabla_Fmcp[1, 1]
    )
    B[:] = nabla_Fmcp
    pass
...
if __name__ == "__main__":
    ...
    pos = np.abs(sol[:, 0])
    velocity = (
        (1 - _kappa * np.sign(sol[:, 0] * sol[:, 1])) * sol[:, 1] * np.sign(sol[:, 0])
    )
