...
namespace two_links_flexible_plugins {
...
inline std::vector<double> param(nDof * 5, 0.);
...
inline auto u_func = [](const Eigen::Ref<const siconos::algebra::SiconosVector> &velocity,
  ...
  double ddv1 =
      ((m11 *
            (m2 * l1 * l2 * cos(q(1)) * velocity(0) * velocity(0) * velocity(1) / 2 +
             m2 * l1 * l2 * sin(q(1)) * velocity(0) * dv0 + K2 * (velocity(1) - velocity(3))) +
  ...
  double qd1 = 0.4 + 0.3 * cos(2 * std::numbers::pi * time / P);
  double qd2 = 0.3 * sin(2 * std::numbers::pi * time / P);
  double qd11 = -(2 * std::numbers::pi / P) * 0.3 * sin(2 * std::numbers::pi * time / P);
  double qd12 = (2 * std::numbers::pi / P) * 0.3 * cos(2 * std::numbers::pi * time / P);
  double qd21 = -(2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) * 0.3 * cos(2 * std::numbers::pi * time / P);
  double qd22 = -(2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) * 0.3 * sin(2 * std::numbers::pi * time / P);
  double qd31 = (2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) * 0.3 * sin(2 * std::numbers::pi * time / P);
  double qd32 = -(2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) * 0.3 * cos(2 * std::numbers::pi * time / P);
  double qd41 =
      (2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) * 0.3 * cos(2 * std::numbers::pi * time / P);
  double qd42 =
      (2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) * 0.3 * sin(2 * std::numbers::pi * time / P);
  ...
  double ddT13 = -l2 * cos(q(0) + q(1)) * (velocity(0) + velocity(1)) *
...
inline auto u1_func = [](const Eigen::Ref<const siconos::algebra::SiconosVector> &velocity,
  ...
  double t3 = (time - param[8]) / 1;

  double qd1 = 0.4 + 0.3 * cos(2 * std::numbers::pi * param[8] / P);
  double qd11 = 0;
  double qd21 = 0;
  double qd31 = 0;
  double qd41 = 0;

  double b0 = 0.3 * sin(2 * std::numbers::pi * param[8] / P);
  double b1 = 0.3 * (2 * std::numbers::pi / P) * cos(2 * std::numbers::pi * param[8] / P);
  double b2 = -3 * (b0 + b1) - 3 * sqrt(param[7]) * alpha;
  double b3 = 2 * (b0 + b1) + 2 * sqrt(param[7]) * alpha;
  ...
  double ki1c = sqrt((qd1 * qd1 + qd2c * qd2c + l1 * l1 + l2 * l2) *
                         (qd1 * qd1 + qd2c * qd2c + l1 * l1 + l2 * l2) -
                     2 * ((qd1 * qd1 + qd2c * qd2c) * (qd1 * qd1 + qd2c * qd2c) +
                          l1 * l1 * l1 * l1 + l2 * l2 * l2 * l2));
  ...
  double xd1c = (qd1 * qd12c - qd2c * qd11) / (qd1 * qd1 + qd2c * qd2c) +
                ((qd1 * qd11 + qd2c * qd12c) * (l1 * l1 - qd1 * qd1 - qd2c * qd2c - l2 * l2)) /
                    (ki1c * (qd1 * qd1 + qd2c * qd2c));
  double yd1c = 2 * (qd1 * qd11 + qd2c * qd12c) / ki1c;
  // double xd2c = (((qd1 * qd21 + qd11 * qd11 + qd2c * qd22c + qd12c * qd12c) * (l1 * l1 - qd1
  // * qd1 - qd2c * qd2c - l2 * l2) - 2 * (qd1 * qd11 + qd2c * qd12c) * (qd1 * qd11 + qd2c *
  // qd12c)) * ki1c * (qd1 * qd1 + qd2c * qd2c) - (qd1 * qd11 + qd2c * qd12c) * (l1 * l1 - qd1
  // * qd1 - qd2c * qd2c - l2 * l2) * (ki1c * 2 * (qd1 * qd11 + qd2c * qd12c) - (qd1 * qd1 +
  // qd2c * qd2c) * 2 * (qd1 * qd11 + qd2c * qd12c) * ki3c / ki1c)) / (ki1c * (qd1 * qd1 + qd2c
  // * qd2c) * ki1c * (qd1 * qd1 + qd2c * qd2c)) + ((qd1 * qd22c - qd2c * qd21) * (qd1 * qd1 +
  // qd2c * qd2c) - (qd1 * qd12c - qd2c * qd11) * 2 * (qd1 * qd11 + qd2c * qd12c)) / ((qd1 *
  // qd1 + qd2c * qd2c) * (qd1 * qd1 + qd2c * qd2c)); double yd2c = 2 * ((qd1 * qd21 + qd11 *
  // qd11 + qd2c * qd22c + qd12c * qd12c) * ki1c * ki1c - (qd1 * qd11 + qd2c * qd12c) * 2 *
  // (qd1 * qd11 + qd2c * qd12c) * ki3c) / ki1c * ki1c * ki1c;
  ...
  param[6] = V1 +
             gamma1 * gamma2 *
                 ((q(0) - xdc) * (q(0) - xdc) + (q(1) - ydc) * (q(1) - ydc) +
                  (q(2) - thetad1) * (q(2) - thetad1) + (q(3) - thetad2) * (q(3) - thetad2)) +
             (q(0) - xd - q(2) + thetad1) * K1 * (q(0) - xd - q(2) + thetad1) / 2 +
             (q(1) - yd - q(3) + thetad2) * K2 * (q(1) - yd - q(3) + thetad2) / 2;
...
inline auto u2_func = [](const Eigen::Ref<const siconos::algebra::SiconosVector> &velocity,
  ...
  double s1 = x1 - qr11;
  double s2 = y1 - qr12;
  double s2bar = y1 + gamma2 * (y + param[7] * alpha);
  ...
  double ddT13 =
      ...
      l2 * sin(q(0) + q(1)) * (velocity(0) + velocity(1)) * (velocity(0) + velocity(1)) *
          gamma1 * s2bar -
      l2 * sin(q(0) + q(1)) * (dv0 + dv1) * gamma1 * s2bar -
      l2 * sin(q(0) + q(1)) * (velocity(0) + velocity(1)) * gamma1 * (y2 + gamma2 * y1) +
      grad12 * gamma1 * (x3 - qr31) + grad22 * gamma1 * (y3 + gamma2 * y2) -
      l2 * cos(q(0) + q(1)) * (velocity(0) + velocity(1)) * gamma1 * (x2 - qr21) -
      l2 * sin(q(0) + q(1)) * (velocity(0) + velocity(1)) * gamma1 * (y2 + gamma2 * y1);
...
inline auto u3_func = [](const Eigen::Ref<const siconos::algebra::SiconosVector> &velocity,
  ...
  double c3 = 2 * (param[5] - 0.7);
  double c2 = -3 * (param[5] - 0.7);
  ...
  double qd31 = (6 * c3) / ((k * P - param[8]) * (k * P - param[8]) * (k * P - param[8]));
  double qd41 = 0;
  double qd2 = 0;
  double qd12 = 0;
  double qd22 = 0;
  ...
  double thetad1 = xd + (T01 + T02 - T03 + grad21 * (Kf * 1000 * param[4] - ld)) / K1;
  double thetad2 = yd + (T11 + T12 - T13 + grad22 * (Kf * 1000 * param[4] - ld)) / K2;
  double thetad11 = xd1 + (dT01 + dT02 - dT03) / K1;
  double thetad12 = yd1 + (dT11 + dT12 - dT13) / K2;
  double thetad21 = xd2 + (ddT01 + ddT02 - ddT03) / K1;
  double thetad22 = yd2 + (ddT11 + ddT12 - ddT13) / K2;
...
inline auto u4_func = [](const Eigen::Ref<const siconos::algebra::SiconosVector> &velocity,
  ...
  double b = -param[10] * ep * P / (4 * 0.3 * std::numbers::pi);

  double qd1 = 0.4 + 0.3 * cos(2 * std::numbers::pi * time / P);
  double qd11 = -0.3 * (2 * std::numbers::pi / P) * sin(2 * std::numbers::pi * time / P);
  double qd21 = -0.3 * (2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) * cos(2 * std::numbers::pi * time / P);
  double qd31 = 0.3 * (2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) * sin(2 * std::numbers::pi * time / P);
  double qd41 =
      0.3 * (2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) * cos(2 * std::numbers::pi * time / P);

  double qd2 =
      0.3 * sin(2 * std::numbers::pi * (k * P + (time - k * P) * sin(b * td + (std::numbers::pi / 2 - b) * td * td)) / P);
  double qd12 =
      0.3 * (2 * std::numbers::pi / P) *
      cos(2 * std::numbers::pi * (k * P + (time - k * P) * sin(b * td + (std::numbers::pi / 2 - b) * td * td)) / P) *
      (sin(b * td + (std::numbers::pi / 2 - b) * td * td) + (time - k * P) *
                                                  cos(b * td + (std::numbers::pi / 2 - b) * td * td) *
                                                  (b / ep + 2 * (std::numbers::pi / 2 - b) * td / ep));
  double qd22 =
      -0.3 * (2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) *
          sin(2 * std::numbers::pi * (k * P + (time - k * P) * sin(b * td + (std::numbers::pi / 2 - b) * td * td)) / P) *
          (sin(b * td + (std::numbers::pi / 2 - b) * td * td) + (time - k * P) *
                                                      cos(b * td + (std::numbers::pi / 2 - b) * td * td) *
                                                      (b / ep + 2 * (std::numbers::pi / 2 - b) * td / ep)) +
      0.3 * (2 * std::numbers::pi / P) *
          cos(2 * std::numbers::pi * (k * P + (time - k * P) * sin(b * td + (std::numbers::pi / 2 - b) * td * td)) / P) *
          (2 * cos(b * td + (std::numbers::pi / 2 - b) * td * td) * (b / ep + 2 * (std::numbers::pi / 2 - b) * td / ep) +
           (time - k * P) *
               (2 * cos(b * td + (std::numbers::pi / 2 - b) * td * td) * (std::numbers::pi / 2 - b) / (ep * ep) -
                sin(b * td + (std::numbers::pi / 2 - b) * td * td) * (b / ep + 2 * (std::numbers::pi / 2 - b) * td / ep) *
                    (b / ep + 2 * (std::numbers::pi / 2 - b) * td / ep)));
  double qd32 =
      -0.3 * (2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) * cos(std::numbers::pi / 2 + 2 * std::numbers::pi * time / P);
  double qd42 = 0.3 * (2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) * (2 * std::numbers::pi / P) *
                sin(std::numbers::pi / 2 + 2 * std::numbers::pi * time / P);

  param[13] = (time - k * P);  // td*ep;
