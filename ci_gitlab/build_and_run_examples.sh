...
ctest -S ${CI_PROJECT_DIR}/ci_gitlab/ctest_driver_examples.cmake \
    -Dmodel=$CTEST_BUILD_MODEL -DALLOW_PARALLEL_BUILD=$PARALLEL_BUILD -DCDASH_SUBMIT=$CDASH_SUBMIT \
    -V --output-junit test_results.xml -DCTEST_BINARY_DIRECTORY=${BUILD_DIR} -DCTEST_SOURCE_DIRECTORY=$CI_PROJECT_DIR/examples \
    --output-log $BUILD_DIR/siconos-examples-ctest.log
